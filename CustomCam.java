...
/**
*  Controls: - Move the mouse to rotate the camera - Mouse wheel for 
zooming in
*  or out
*/
...
/**
*  Gets the zoom speed. The speed is a multiplier to 
...
/**
*  Registers the FlyByCamera to receive input events from the 
provided
*  Dispatcher.
*
*  @param inputManager
...
Vector3f click3d = cam.getWorldCoordinates(new 
Vector2f(click2d.x, click2d.y), 0f).clone();
Vector3f dir = cam.getWorldCoordinates(new 
Vector2f(click2d.x, click2d.y), 
...
cam.setLocation(new Vector3f(lookAt.x, lookAt.y, lookAt.z + 
dist));
pos = cam.getLocation();
...
lookAt = centroid.clone();
cam.setLocation(new Vector3f(lookAt.x, lookAt.y + dist, 
lookAt.z));
pos = cam.getLocation();
...
lookAt = centroid.clone();
cam.setLocation(new Vector3f(lookAt.x + dist, lookAt.y, 
lookAt.z));
pos = cam.getLocation();
cam.lookAt(lookAt, Vector3f.UNIT_Y);
upVec = Vector3f.UNIT_Y;
...
lookAt = centroid.clone();
cam.setLocation(new Vector3f(CamD[dir].x + lookAt.x, CamD[dir].y + 
lookAt.y, CamD[dir].z + lookAt.z));
pos = cam.getLocation();
cam.lookAt(lookAt, Vector3f.UNIT_Y);
upVec = Vector3f.UNIT_Y;
resetZoom();
